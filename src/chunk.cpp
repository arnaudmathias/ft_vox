...
float mapp(float unscaledNum, float minAllowed, float maxAllowed, float min,float  max) {
  return (maxAllowed - minAllowed) * (unscaledNum - min) / (max - min) + minAllowed;
}
enum Color {BLACK = 0, BLUE = 1, GREEN = 2, CYAN = 3, RED = 4, VIOLET = 5, KAKI = 6, WHITE = 7};
...
Color get_best_color(glm::vec3 earth_color) {
	...
	//std::cout << earth_color.x << "|" << earth_color.y << "|"<< earth_color.z << std::endl;
...
void ChunkManager::Draw_earth() {
	...
	int w, h;
	int texChannels = 0;
	stbi_uc *pixels = stbi_load("./textures/earth.jpg", &w, &h,
	...
	for (int x = -size; x < size; x++) {
		for (int y = -size; y < size; y++) {
			for (int z = -size; z < size; z++) {
				...
				glm::vec3 color(pixels[(v * w + u) * 4], pixels[(v * w + u + 1) * 4], pixels[(v * w + u + 2) * 4]);
				int idmax = 0;
				float mindist = 9999;
